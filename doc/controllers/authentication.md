...
```ts
try {
  const response = await authenticationController.customAuthentication();
...
```ts
try {
  const response = await authenticationController.oAuthBearerToken();
...
This method returns an [`ApiResponse`](../../doc/api-response.md) instance. The `result` property of this instance returns the response data which is of type [`ServiceStatus`](../../doc/models/service-status.md).

## Example Usage

```ts
try {
  const response = await authenticationController.oAuthClientCredentialsGrant();
...
```ts
try {
  const response = await authenticationController.oAuthAuthorizationGrant();
...
```ts
try {
  const response = await authenticationController.customQueryOrHeaderAuthentication();
...
```ts
try {
  const response = await authenticationController.basicAuthAndApiHeaderAuth();
...
```ts
try {
  const response = await authenticationController.oAuthGrantTypesORCombinations();
...
```ts
try {
  const response = await authenticationController.multipleAuthCombination();
...
```ts
try {
  const response = await authenticationController.noAuth();
